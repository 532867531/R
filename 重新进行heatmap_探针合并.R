...
for(amethods in  Cluster_Method){
 for(onemethod in Dist_Methods){
      ...
      file0=paste(out_put_dir,amethods,onemethod,ak_n,paste(para_List,collapse = "_"),"tree_plot",".tiff",sep="_");print(file)
      
      dist=dist(as.matrix(t(data_signatire)),method = "euclidean")
      a=hclust(dist,method = "complete")
      b=cutree(a,k=5)
      hccut=d=as.data.frame(b)
      colnames(hccut)="group_index"
      hccut[,"SampleIndex"]=rownames(hccut)
      library(sqldf)
      r=sqldf("select count(`group_index`) from hccut group by `group_index`")
      ##调取某个基因的数据
      SAMPLES=rownames(hccut);getGene="CXCL17"
      SAMPLES_EXP=as.data.frame(t(rna_seq_data[getGene,SAMPLES]))
      SAMPLES_EXP[,"SampleIndex"]=rownames(SAMPLES_EXP)
      ...
      ##开始绘图
      if(TRUE){
        file=paste(out_put_dir,amethods,onemethod,k_n0,paste(para_List,collapse = "_"),"box_plot",".tiff",sep="_")
        tiff(filename = file,width = 1000, height = 800, units = "px")
        #win.graph()
        # win.graph()
        # p<-ggplot(data=data_plot, aes(x=group_index,y=getGene))
